...
class LeafyTreeTest extends TreeTest {
    ...
    void growTesting() {
        ...
        assertEquals(0,leafyTree.GetLeavesNumber(),"Needles should be 0 before first growing");

        leafyTree.grow(2);
        assertEquals(2, tree.GetBranchesNumber(), "Branches should be 2 before first growing");
        assertEquals((2*2),leafyTree.GetLeavesNumber(),"Needles should be 4 before first growing");

        leafyTree.grow(3);
        assertEquals((2+3), tree.GetBranchesNumber(), "Branches should be 2 before first growing");
        assertEquals((2*2+3*2),leafyTree.GetLeavesNumber(),"Needles should be 4 before first growing");
    }
